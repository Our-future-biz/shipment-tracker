...
class AutomationLogRepository {
  async listByShipmentId(shipmentId: string, limit = 100) {
    return db
      ...
      .where(eq(automationLogTable.shipmentId, shipmentId))
      .orderBy(desc(automationLogTable.createdAt))
      .limit(limit);
  }

  async create(data: { shipmentId: string; ruleName: string; action: string; details?: unknown; triggeredById?: string | null }) {
