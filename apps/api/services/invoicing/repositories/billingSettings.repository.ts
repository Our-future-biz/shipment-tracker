...
class BillingSettingsRepository {
  async getByShipmentId(shipmentId: string) {
    const [row] = await db
      ...
      .where(eq(billingSettingsTable.shipmentId, shipmentId))
      .limit(1);
    return row ?? null;
  }

  async upsert(shipmentId: string, data: Partial<{ billingCurrency: string; roe: string; quoteRef: string }>) {
    const existing = await this.getByShipmentId(shipmentId);
    if (existing) {
      const [row] = await db
    ...
    const [row] = await db.insert(billingSettingsTable).values({ shipmentId, ...data }).returning();
    return row!;
  }
}
